...
class BankSystem:
    ...
    def withdraw(self, account_no, amount):
        if account_no in self.accounts:
            if not self.bankrupt:
                ...
                print("The bank is bankrupt.There is no money for loan!!")
        else:
            print("Invalid account")
    ...
    def take_loan(self, ac_no, amount):
        if self.loan_available:  
            if not self.bankrupt:  
                if ac_no in self.accounts:  
                    if self.accounts[ac_no]["loan_count"] < self.loan_limit:  # Check if the user hasn't reached the loan limit
...
class Admin:
    ...
    def change_bankrupt_status(self, status):
        self.BankSystem.toggle_bankrupt_status(status)
